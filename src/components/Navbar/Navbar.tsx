...
import ButtonResizeNavbar from './ButtonResizeNavbar';
...
const Navbar = (props: Props) => {
  ...
  const isPathLoginRegister =
    pathName.includes('/signin') || pathName.includes('/signup');

  const {status, data: session} = useSession();

  const actionResizeNavbar = () => {
    const navbar: HTMLDivElement | null = document.querySelector('.navbar>div');
    const navHeight = navbar ? navbar.style.height : 0;
    const heightChange = navHeight == '115px' ? '250px' : '115px';

    navbar && (navbar.style.height = heightChange);
  };

  useEffect(() => {
    const navbar: HTMLDivElement | null = document.querySelector('.navbar>div');

    if (navbar && navbar.style.height != '115px') {
      windowWidth > 767 && (navbar.style.height = '115px');
    }
  }, [windowWidth]);

  return (
    <nav className={s.navigation + ' container-base navbar '}>
      <div
        style={{height: '115px'}}
        className="py-8 flex max-md:flex-col items-center gap-6 md:justify-between duration-300 overflow-hidden "
      >
        <div className="max-md:w-full flex items-center justify-between">
          ...
          <ButtonResizeNavbar actionResizeNavbar={actionResizeNavbar} />
        </div>
        {status !== 'loading' && (
          <div className="flex max-md:flex-col items-center max-md:mt-2 max-md:gap-6">
            <ul className="flex-center gap-8">
              <li className={pathName.toString() == '/' ? 'active' : ''}>
                <Link href={'/'}>Home</Link>
              </li>
              ...
              <li
                className={pathName.includes('/rewards') ? 'active' : ''}
                onClick={() => {}}
              >
            ...
            {status == 'authenticated' ? (
              ...
              <div className="flex-center">
                <Link href={'/signup'}>
                  <Button className="hover:text-success ">Sign up</Button>
                </Link>

                <Link href={'/signin'}>
                  <Button className="bg-success text-white px-8">
                    Sign In
                  </Button>
                </Link>
              </div>
